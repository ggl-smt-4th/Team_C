...
contract('Payroll', (accounts) => {
  ...
  const employee_second=accounts[2];
  ...
  it("Test call removeEmployee() to remove an empty employee by owner", () => {
    return payroll.removeEmployee(employee_second, {from: owner}).then(() => {
      assert(false, "Should not be successful");
    }).catch(error => {
      assert.include(error.toString(), "Error: VM Exception", "Cannot call removeEmployee() to remove an empty employee by owner");
    });
  });
