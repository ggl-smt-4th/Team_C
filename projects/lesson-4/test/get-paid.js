...
contract('Payroll', function (accounts) {
  ...
  it("Test call getPaid() with not enough fund", function () {
      var payroll;
      return Payroll.new.call(owner, {from: owner, value: web3.toWei(0.1, 'ether')}).then(instance => {
          payroll = instance;
          return payroll.addEmployee(employee, salary, {from: owner});
      }).then(() => {
          return payroll.calculateRunway();
          ...
          assert.include(error.toString(), "Error: VM Exception", "Should not call getPaid() sucessfully with not enough fund");
      });
  });
});