...
contract('Payroll', function (accounts) {
    ...
    it("Test call getPaid() with not enough fund", function () {
        var payroll;
        return Payroll.new.call(owner, {from: owner, value: web3.toWei(fund/40, 'ether')}).then(instance => {
            payroll = instance;
            return payroll.addEmployee(employee, salary, {from: owner});
        }).then(() => {
            return payroll.calculateRunway();
            ...
            assert.include(error.toString(), "Error: VM Exception", "Should not getPaid() with not enough fund");
        });
    });
});
